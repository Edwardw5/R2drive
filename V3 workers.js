...
// Version 5.0: Integrated media/document viewer modal, SPA-like experience.
    ...
    async fetch(request, env, ctx) {
        ...
        switch (method) {
            case "GET":
                ...
                // The /preview/ route is now effectively handled by the frontend viewer, but we keep it for direct access or right-click "open in new tab".
...
function generateHTML({ prefix, folders, files, isLoggedIn, env, totalSize }) {
    ...
    const fileRows = files.map(file => `<tr class="item-row" data-key="${escapeHtml(file.key)}" data-name="${escapeHtml(file.name)}" data-type="file" data-size="${escapeHtml(file.size)}" data-uploaded="${escapeHtml(file.uploaded)}"><td><input type="checkbox" class="item-checkbox"></td><td><a href="javascript:void(0)" class="item-link viewer-link"><span class="icon">${file.icon}</span>${escapeHtml(file.name)}</a></td><td>${file.size}</td><td>${file.uploaded}</td><td>文件</td></tr>`).join('');
